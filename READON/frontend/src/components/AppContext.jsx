...
export const AppContextProvider = ({ children }) => {
    ...
    const fetchSeller = async () =>{
        try {
            const {data} = await axios.get(`${process.env.REACT_APP_API_URL}/api/seller/is-auth`);
            if(data.success){
                setIsSeller(true)
            }
            else{
    ...
    const fetchUser = async () =>{
        try {
            const {data} = await axios.get(`${process.env.REACT_APP_API_URL}/api/user/is-auth`);
            if(data.success){
                setUser(data.user)
  ...
  useEffect(() => {
    const updateCart = async () => {
        try {
            const { data } = await axios.post(`${process.env.REACT_APP_API_URL}/api/cart/update`, { userId: user._id, cartData: cartItems})
            if (!data.success) {
                toast.error(data.message);
            }
        } catch (error) {
