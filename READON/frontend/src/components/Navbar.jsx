...
const Navbar = () => {
    ...
    const handleLogout = async () => {
        try {
            const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/user/logout`)
            if (data.success) {
        ...
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24
         xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="flex items-center gap-4 sm:hidden">
                {/* Cart Icon */}
                <div className="relative cursor-pointer">
                    <img onClick={() => navigate("cart")} src={assets.nav_cart_icon} alt="cart"
            ...
            {open && (
                <div className={`${open ? "flex" : "hidden"} absolute top-[60px] left-0 w-full
                 bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
