...
const ProductDetails = () => {
    ...
    const { products, addToCart, currency, navigate } = useAppContext();
    const { id } = useParams();
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                            ...
                            <img src={i < 4 ? assets.star_icon : assets.star_dull_icon}
                                className="md:w-4 w-3.5" alt="STAR ICON" />
                    ...
                    <div className="flex items-center mt-10 gap-4 text-base">
                        ...
                        className="w-full py-3.5 cursor-pointer font-medium bg-gray-100
                         text-gray-800/80 hover:bg-gray-200 transition" >
                            Add to Cart
                        </button>
                        <button onClick={() => {addToCart(product.id); navigate('/cart')}}  
                        ...
                        <div className="flex flex-col items-center mt-20">
                            <div className="flex flex-col items-center w-max">
                                <p className="text-3xl font-medium">Realated Products</p>
                                <div className="w-20 h-0.5 bg-primary rounded-full mt-2"></div>
                            </div>
                            <div className=" grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 
                             lg:grid-cols-5 mt-6 gap-3 md:gap-6">
                                {realatedProducts.filter((product)=> product.inStock).map
                                ((product, index)=>(
                                    <ProductCard key={index} product={product} />
                                ))}
                            </div>
                            <div onClick={()=> {navigate('/products'); scrollTo(0,0)}}
