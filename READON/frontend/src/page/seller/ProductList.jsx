...
const ProductList = () => {
  ...
  const {products, currency, axios, fetchProducts, toast} = useAppContext();

  const toggleStock = async (id, inStock) =>{
    try {
        const {data} =  await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/product/stock`, {id, inStock});
        if (data.success){
            fetchProducts();
                                ...
                                <tr key={product._id} className="border-t border-gray-500/20">
                                    ...
                                    <td className="px-4 py-3">
                                        <label className="relative inline-flex items-center cursor-pointer text-gray-900 gap-3">
                                            ...
                                            <span className="dot absolute left-1 top-1 w-5 h-5
