...
const CartPage = () => {
    ...
    const getUserAddress = async () =>{
        try {
const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/address/get?userId=${user._id}`);
            if(data.success){
    ...
    const placeOrder = async () => {
        try {
            ...
            if(paymentMethod === 'COD'){
                const {data} = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/order/cod`, {
                    userId: user._id,
                ...
                //pace order with stripe or online payment
                 const {data} = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/order/stripe`, {
                    userId: user._id,
                    items: cartArray.map(item => ({product: item._id, quantity: item.quantity})),
                    address: selectedAddress._id
                })
...
const finalTotal = Number((totalPrice + tax).toFixed(2));
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-primary">{getTotalItems()}</span>
                </h1>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                ...
                                <p onClick={() => navigate('/addAddress')} className="text-primary 
                                text-center cursor-pointer p-2 hover:bg-primary-dull   ">
                                    Add address
                                </p>
                            </div>
                        )}
                ...
                <button onClick={placeOrder}
                    className="w-full py-3 mt-6 cursor-pointer 
